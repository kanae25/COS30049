...
The ShieldMail application integrates a Naive Bayes spam detection model trained in Assignment 2 and packaged for standalone execution in Assignment 3. The model is loaded at application startup and provides real-time spam classification through the FastAPI backend.

### 4.1 Model Training and Packaging

The spam detection model is trained using `train_and_save_models.py`, which creates a scikit-learn Pipeline combining TF-IDF vectorization (10,000 features, English stop words, bigrams) with Naive Bayes classification. The script tests both MultinomialNB and BernoulliNB variants, selecting the best model based on F1 score. The trained pipeline is saved as `spam_detection_model.pkl` using joblib, along with metadata (`model_metadata.json`) containing performance metrics (accuracy: 0.965, F1: 0.907).

**Model Training Example** (`train_and_save_models.py`, lines 105-109):
```python
# File: train_and_save_models.py (lines 105-109)
pipeline = Pipeline([
...
The `ModelService` class (`backend/model_service.py`) automatically loads the packaged model from `backend/models/` when the FastAPI application starts (`backend/main.py`, line 46). The service handles prediction by applying TF-IDF vectorization through the pipeline and classifying text using the Naive Bayes classifier, returning spam/legitimate classification with probability scores.

**Prediction Method** (`backend/model_service.py`, lines 100-118):
```python
# File: backend/model_service.py (lines 100-118)
...
The prediction endpoint (`POST /api/predict`) integrates with `model_service.predict()` to provide real-time spam classification. Assignment 3 operates as a standalone application with the model packaged in `backend/models/`, requiring no external dependencies on Assignment 2 at runtime.
